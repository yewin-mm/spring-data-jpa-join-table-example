...
@Entity
@Table(name ="users") // if not define, it still ok. But you can't use the table name with user and it can conflict (got error while running) with table which already in postgres db.
...
public class User {
    ...
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY) // the id will auto generate and so, no need to put id when saving into database.
    private Long id;
    ...
    @ManyToOne(fetch = FetchType.LAZY) // many user can have one role and use fetch type to lazy to avoid EAGER loading and to get good performance
